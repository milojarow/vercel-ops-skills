...
- **`vercel env add` marks the variable sensitive.** A later `vercel env pull` then returns
  `[SENSITIVE]` instead of the value. This is not corruption and not a lost secret. To confirm what
  actually shipped, inspect the deployed artifact — for a `NEXT_PUBLIC_*` variable the value is
  present in the served HTML. Do not declare the value unknowable.
